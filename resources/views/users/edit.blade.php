...
<div class="w-full px-6 py-6 mx-auto">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
      <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
        <h6 class="dark:text-white text-xl">Edit User</h6>
          <div class="max-w-xl">
            ...
            <div class="mb-4">
              ...
              <x-text-input id="nip" name="nip" type="number" class="mt-1 block w-full" :value="old('nip', $user->nip)" autofocus autocomplete="nip" />
            ...
            {{-- <div class="mb-4">
              ...
              <select name="jabatan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                ...
                @foreach ($roles as $item)
                  <option value="{{$item->name}}" {{$item->name == $user->roles()->first()->name ? 'selected' : ''}}>{{Str::upper($item->name)}}</option>
                @endforeach
              </select>
            </div> --}}
            <div class="mb-4">
                <x-input-label for="password" :value="__('Password')" />
                <p class="text-xs text-red-600">jangan diisi jika tidak ingin diubah</p>
